...
public class PersonageService {
	...
	public void savePersonages(List<Personage> personages, PersonageCall lastCall) {	
		personageRepository.save(personages);
	}
	
	public void cleanOldPersonages(PersonageCall lastCall) {
		synchronized (personageRepository) {
	...
	public List<String> searchByWordStarting(String searchWord) {
		...
		List<Personage> personages = personageRepository.findAllByCreatedAtBetweenAndNameContains(lastFinishedCall.getCreatedAt(), lastFinishedCall.getUpdatedAt(), searchWord);
		...
		List<String> inFirstWordList = personages.stream()
				.map(Personage::getName)
				.filter(name -> inFirstWordRegexPattern.matcher(name).find())		
				.sorted((s1,s2) -> {
		...
		List<String> notFirstWordList = personages.stream()
				.map(Personage::getName)
				.filter(name -> inFirstWordList.contains(name) == false)
				.sorted((s1,s2) -> {
					Matcher matcher1 = stringSearchPattern.matcher(s1);
					matcher1.find();
					Matcher matcher2 = stringSearchPattern.matcher(s2);
